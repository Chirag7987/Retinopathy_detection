...
def train(cfg: DictConfig) -> None:
    ...
    # Convert train_csv_path to absolute path
    cfg.train_csv_path = abspath(cfg.train_csv_path)
    cfg.val_csv_path = abspath(cfg.val_csv_path)

    # Check if train CSV file exists
    if not exists(cfg.train_csv_path):
        raise FileNotFoundError(f"Train CSV file '{cfg.train_csv_path}' not found.")
    
    # Check if validation CSV file exists
    if not exists(cfg.val_csv_path):
        raise FileNotFoundError(f"Validation CSV file '{cfg.val_csv_path}' not found.")
    
    # Print paths for debugging
    print(f"Train CSV Path: {cfg.train_csv_path}")
    print(f"Validation CSV Path: {cfg.val_csv_path}")
